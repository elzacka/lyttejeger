...
export function AudioPlayer({ episode, onClose }: AudioPlayerProps) {
  ...
  const togglePlayPause = useCallback(() => {
    ...
    if (isPlaying) {
      ...
      // CRITICAL for iOS: Call play() synchronously - no await!
      // The play() promise rejection is handled asynchronously, but the call itself is sync
      audio.play().catch((error) => {
        console.error('Play failed:', error);
        // Try recovery: load and play again
        audio.load();
        audio.play().catch(() => {
          setAudioError(true);
        });
